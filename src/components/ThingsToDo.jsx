  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-1 mt-10">
      {thingsToDoList.map((item, index) => (
        <div
          key={index}
          className="relative w-full h-auto rounded-[15px] flex flex-col items-start"
        >
          <img
            src={item.image}
          ...
          <div className="mt-1 w-full px-1">
            <p className="text-[20px] font-medium leading-[30px] text-left text-[#152C5B]">
              {item.name}
            </p>
            <p className="text-[15px] font-light leading-[22.5px] text-left text-[#152C5B] mt-1">
              {item.category}
            </p>
          </div>
        </div>
